...
void followLineRight(float Vnom, float K, float Ke)
{
  ...
  if(Vrot>0) Vnom=Vrot; 
  else if(Vrot<=0 || fabs(IRLine.pos_right)) Vnom=0;
  robot.v = Vnom;
...
void followLineLeft(float Vnom, float K, float Ke)
{
  ...
  if(Vrot>0) Vnom=Vrot; 
  else if(Vrot<=0 || fabs(IRLine.pos_left)) Vnom=0;
  robot.v = Vnom;
...
#define Vmed 80
#define Varm 60
#define kapa -2.0 //ja alterei, testar amanha
#define kaparm -1.0
#define ke -1.0 //vira muito rapido para a caixa/
void control(void)
{
    ...
    if (robot.state == 0) {         // Robot Stoped            
      ...
      else if (robot.state==2 && tis>1500) setState(3); //volta para trás até detetar a direita
      ...
      else if (robot.state==5 && tis>=1000 ) setState(6); //entrar no armazem
      ...
      else if (robot.state==8 && tis>=1000) setState(9); //cruzamento
      ...
      else if (robot.state==13 && tis>=1000) setState(14); //ultrapassar o cruzamento
      ...
      if (robot.state == 0) {         // Robot Stoped            
        ...
        robot.solenoid_state = 1;
        followLineRight(Vmed-40*(IRLine.crosses==5), kapa, ke);
      } else if (robot.state == 5) {
        robot.solenoid_state = 1;
        if (tis<1000) moveRobot(0, 0); 
        else followLineRight(Vmed, kapa,ke);
        ...
        robot.solenoid_state = 1;
        followLineLeft(Vmed-40*(IRLine.crosses==6), kapa,ke);
