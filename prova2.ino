...
void control(void)
{
  int cont_ini=0, RFID=1, MA1=0;
    ...
    else if (robot.state==2 && IRLine.crosses >=1 && RFID==1){  //recua ate detetar a direita
      setState(3);
    }
    else if(robot.state == 3 && tis > 1600 && IRLine.total > 1500) { //vira 180 e avança pela esquerda
      if(MA1==0) setState(4);
      //else if(MA1==1) setState();
     }
     else if(robot.state == 4 && IRLine.crosses >= 1) { 
      setState(6);
     }
...
//    }
    ...
    } else if (robot.state == 2) {  // Go back with the box 
      robot.solenoid_state = 1;
      moveRobot(-60, 0);
      
    } else if (robot.state == 3) {  // Turn 180 degrees
      robot.solenoid_state = 1;
      moveRobot(0, 50);
    
    } else if (robot.state == 4) {  // segue pela esquerda
      robot.solenoid_state = 1;
      followLineLeft(60, -1.0);
    ...
    } else if (robot.state == 7) {  //recua da maquina
      robot.solenoid_state = 0;
      moveRobot(-60, 0);
      MA1++;

    } else if (robot.state == 8) { //segue a linha pela direita 
      robot.solenoid_state = 0;
      followLineRight(80, -2.0);  
          
    } else if (robot.state == 9) { //segue a linha pela esquerda 
      robot.solenoid_state = 0;
