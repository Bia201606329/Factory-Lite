...
void control(void)
{
    ...
    if (robot.state == 0) {         // Robot Stoped            
      ...
      else if(robot.state==4 && IRLine.crosses>=7-cont_arm) setState(5); //andar até o cruzamento
      ...
      else if (robot.state==7 && tis>2000) setState(8); //entrar no armazem
      ...
      else if (robot.state==11 && IRLine.crosses>=3) setState(12); //segue até à interseção das peças
      ...
      if (robot.state == 0) {         // Robot Stoped            
        ...
        robot.solenoid_state = 1;
        if(tis<1500) followLineRight(40, -1.0);  
        else  followLineRight(60, -1.0);
